...
def parse_drink_info(text: str) -> Optional[dict]:
    ...
    patterns_with_numbers = [
        ...
        r'(\d+)\s*(?:г|грамм|мл|литр|л|стакан|стакана|стаканов|банка|банки|банок|бутылка|бутылки|бутылок|бутылку|бутылкой|рюмка|рюмки|рюмок|рюмку|рюмкой|пинта|пинты|пинт)\s+(?:пива|водки|вина|виски)',
        # Добавляем паттерны для сообщений с временными указаниями
        r'(?:сегодня|только что|сейчас|недавно|вчера|утром|вечером|днем|ночью)\s+(?:выпил|выпила)\s+(\d+)\s*(?:г|грамм|мл|литр|л|стакан|стакана|стаканов|банка|банки|банок|бутылка|бутылки|бутылок|бутылку|бутылкой|рюмка|рюмки|рюмок|рюмку|рюмкой|пинта|пинты|пинт)',
    ]
